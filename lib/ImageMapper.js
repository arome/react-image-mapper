...
var ImageMapper = (function (_Component) {
	...
	_createClass(ImageMapper, [{
		...
		value: function renderPath() {
			...
			var circle = _props$path.circle;
			var line = _props$path.line;
			var steps = _props$path.steps;
			...
			return steps.length > 0 && steps.map(function (step, index) {
				...
				var from = _this5.computeCenter(fromArea);
				var toArea = map.areas[step[1]];
				var to = _this5.computeCenter(toArea);
		...
		value: function renderAreas() {
			...
			return this.getExtendedAreas().map(function (extendedArea, index) {
				return _react2['default'].createElement('area', {
					...
					onMouseDown: _this6.mouseDown.bind(_this6, extendedArea, index),
					onMouseUp: _this6.mouseUp.bind(_this6, extendedArea, index),
...
ImageMapper.propTypes = {
	...
	path: _propTypes2['default'].shape({
